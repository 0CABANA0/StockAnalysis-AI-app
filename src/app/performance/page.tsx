...
export default async function PerformancePage() {
  ...
  const holdings = portfolios
    .map((p) => {
      ...
      const currentPrice = getLatestBuyPrice(txs);
  ...
  // 시장별 분포
  const marketDistribution = holdings.reduce(
    (acc, h) => {
      acc[h.market] = (acc[h.market] ?? 0) + h.pnl.marketValue;
      return acc;
    },
      ...
      <main className="mx-auto max-w-7xl px-4 py-8">
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <Card>
            ...
            <CardContent>
              {Object.keys(marketDistribution).length === 0 ? (
                <p className="text-muted-foreground py-8 text-center text-sm">
                  보유 종목이 없습니다.
                </p>
              ) : (
                <div className="space-y-3">
                  {Object.entries(marketDistribution).map(([market, value]) => {
                    const pct =
                      totalMarketValue > 0
        ...
        <p className="text-muted-foreground mt-6 text-center text-xs">
          * 현재가는 최근 매수 가격 기준이며, 실제 수익과 차이가 있을 수
          있습니다. 투자 판단의 책임은 본인에게 있습니다.
        </p>
      </main>
    </div>
  );
